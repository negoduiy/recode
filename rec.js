import { toggleBlur } from './navigation.js';
...
export function initDocumentation() {
    // 1. Инициализируем аккордеоны и вешаем обработчик клика (всегда, без привязки к URL)
...
function initResponsiveDetails() {
    // 1. Первичная настройка состояний при загрузке страницы
    document.querySelectorAll('.wp-block-details').forEach(details => {
        const p = details.querySelector('p');
        if (!p) return;

        // Принудительно открываем нативно для обхода блокировок отображения в Chromium
        if (details.hasAttribute('open')) {
            details.classList.add('active');
            p.style.height = 'auto';
        } else {
            details.setAttribute('open', '');
            p.style.height = '0px'; // Фиксируем инлайн-стиль для JS
        }
    });

    // 2. Глобальное делегирование клика на ВЕСЬ элемент .wp-block-details
    document.addEventListener('click', (e) => {
        const details = e.target.closest('.wp-block-details');
        if (!details) return;

        // Если пользователь просто выделяет текст внутри параграфа, не закрываем спойлер
        if (window.getSelection().toString() && e.target.closest('p')) return;

        const p = details.querySelector('p');
        if (!p) return;

        e.preventDefault(); // Блокируем нативное поведение браузера

        const isActive = details.classList.contains('active');

        if (isActive) {
            // --- ЛОГИКА ЗАКРЫТИЯ ---
            // Фиксируем точную текущую высоту перед анимацией закрытия
            p.style.height = p.getBoundingClientRect().height + 'px';
            p.offsetHeight; // Принудительный reflow
            
            details.classList.remove('active');
            
            // Обертка в requestAnimationFrame гарантирует, что браузер 
            // успеет применить удаление класса .active перед тем, как мы обнулим высоту в JS
            requestAnimationFrame(() => {
                p.style.setProperty('height', '0px', 'important');
            });
        } else {
            // --- ЛОГИКА ОТКРЫТИЯ ---
            // Перед открытием временно убираем инлайновый !important, чтобы задать точную высоту в пикселях
            p.style.removeProperty('height'); 
            
            // Измеряем чистый scrollHeight контента
            const targetHeight = p.scrollHeight;
            
            // Возвращаем контролируемую высоту для старта анимации
            p.style.height = '0px';
            ...
            // После окончания анимации переводим в auto, чтобы текст оставался адаптивным
...
async function loadArticleContent(articleElement) {
    ...
    try {
        ...
        // 💡 ИСПРАВЛЕНО: Удаляем все HTML-комментарии WordPress (Gutenberg блоки)
...
function activateArticle(element) {
    ...
    if (!element.classList.contains('gyasdjlk')) {
        element = document.querySelector(`#mainulist [name="${name}"]`);
        //routeArticleFromUrl();
    }
